...
def parse_personal_data(df):
    ...
    try:
        ...
        if len(unmapped) > 0:
            st.warning(f"⚠️ 다음 카테고리는 매핑되지 않아 분석에서 제외됩니다: {', '.join(unmapped)}")
...
def page_preprocessing():
    st.markdown("<h1 class='main-header'>🛠️ 전처리 및 피처 엔지니어링</h1>", unsafe_allow_html=True)
    ...
    **주요 컬럼:**
    ...
    # ==========================================
    st.markdown("### 🏷️ Step 2: MCC 코드를 6개 카테고리로 변환")
    ...
    with col_filter1:
        st.markdown("#### 1️⃣ 소비 분류 관련없는 거래 제거")
        st.markdown("""
        **제거 대상**:
        - 이체 (계좌이체, 송금 등)
        - 기타 (분류 불가능한 거래)
    ...
    # Step 4: 피처 생성
    # ==========================================
    st.markdown("### ⚙️ Step 4: ML 모델용 피처 생성")
...
verbosity: 1
        ...
        st.warning("⚠️ **Depth 7 이상에서 과적합 발생** → 최적값 6 선택")
    ...
    with tab4:
        ...
        model_comparison = {
            ...
            'Accuracy (%)': [45.1, 45.9, 49.3, 48.0, 47.5],
            'Macro F1 (%)': [38.0, 44.9, 42.1, 46.0, 45.2],
            '학습 시간': ['25분', '8분', '12분', '10분', '20분'],
            '데이터 크기': ['6.4M', '6.4M', '6.4M', '3.9M', '6.4M'],
            '특징': [
                'CPU 기반, 느림',
                'GPU 가속 적용',
                'GPU RF, 높은 Acc',
            ...
            if df_processed is not None:
                ...
                with tab2:
                    ...
                    if len(df_processed) < 10:
                        st.warning("⚠️ 데이터가 너무 적습니다. 최소 10건 이상의 거래가 필요합니다.")
                    else:
